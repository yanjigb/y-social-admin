...
const Login: FC<LoginProps> = ({ ThemeChanger }: any) => {
  ...
  useEffect(() => {
    const storedEmail = localStorage.getItem(LocalStorageKeys.EMAIL);
    if (storedEmail) {
  ...
  const routeChange = () => {
    const path = getDynamicRoute(RouteNames.CRM);
    navigate(path);
  };

  const Login1 = () => {
    if (data.email === "admin@gmail.com" && data.password === "Aa@123456") {
      localStorage.setItem(LocalStorageKeys.EMAIL, data.email); // Store email in localStorage
      routeChange();
    } else {
      setError("The Auction details did not match");
      setData({ email: "", password: "" }); // Clear inputs on error
    }
  };
        ...
        <div className="authentication authentication-basic items-center h-full text-defaultsize text-defaulttextcolor">
          <div className="grid grid-cols-12">
            ...
            <div className="xxl:col-span-4 xl:col-span-4 lg:col-span-4 md:col-span-6 sm:col-span-8 col-span-12">
              ...
              <div className="box">
                <nav
                  className="!block firebase-data mt-6 bg-light mx-auto p-2 rounded-md"
                  aria-label="Tabs"
                >
                ...
                <div
                  className="box-body !p-[3rem] hidden"
                  role="tabpanel"
